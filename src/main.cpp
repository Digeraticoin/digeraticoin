...
#if defined(NDEBUG)
# error "Digeraticoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xda9b726fd30c5de895c9cd47a219fd5fcfe04db63d9cffdfafe06590e705526d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Digeraticoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Digeraticoin Signed Message:\n";
...
{
    // Digeraticoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Digeraticoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Digeraticoin: 1 day
static const int64 nTargetSpacing = 3 * 60; // Digeraticoin: 3 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Digeraticoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Digeraticoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6a1bb4385b346b7c58b8cfaae7e1f0c029e1f6aef2926b0fdd315cd637f6b7b8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b8c5ed8eb71bfda8ed395521a3e5146f4a9dcdb51604c6ac962c2731bed9d3702121c43edc141074981491bc9c968029389e710f1e78ff08ec7f9aa913a9f52d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1518346029;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084770448;

        if (fTestNet)
        {
            block.nTime    = 1518345930;
            block.nNonce   = 386200569;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x2efc86778a57088472ef7f15fe87c228919328d85aeb6df096a137d9a3ddba15"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf5, 0xc2, 0xb9, 0xd1 }; // Digeraticoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DigeraticoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DigeraticoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DigeraticoinMiner(CWallet *pwallet)
{
    printf("DigeraticoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("digeraticoin-miner");
    ...
    try { loop {
        ...
        printf("Running DigeraticoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DigeraticoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DigeraticoinMiner, pwallet));
}
